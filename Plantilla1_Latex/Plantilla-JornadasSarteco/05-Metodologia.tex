...
\subsection{Plataforma experimental (compilador, Intel OneApi)}
Las pruebas se llevaron a cabo utilizando el compilador de C++ \texttt{icpx} perteneciente a la suite Intel oneAPI, ejecutándose sobre un procesador de arquitectura AMD. Para estabilizar las métricas del procesador, se utilizó una imagen PGM sintética de $256 \times 256$ píxeles, la cual fue replicada en memoria 800 veces para generar una carga de trabajo robusta.

\subsection{Versiones implementadas (escalar, automática, guiada, explícita)}
Se evaluaron cuatro variantes del kernel:
\begin{itemize}
    \item \textbf{Escalar:} Línea base compilada con \texttt{-fno-vectorize}.
    \item \textbf{Automática:} Uso exclusivo de \texttt{\_\_restrict} y nivel de optimización \texttt{-O3}.
    \item \textbf{Guiada:} Paralelismo forzado mediante la directiva OpenMP \texttt{\#pragma omp simd}.
    \item \textbf{Explícita:} Inyección manual de intrínsecas AVX2 mediante \texttt{<immintrin.h>}.
\end{itemize}

\subsection{Descripción del kernel}
El kernel numérico objetivo de este estudio calcula la magnitud del gradiente de una imagen utilizando el operador Sobel. Se aisló el ciclo computacionalmente intensivo para medir exclusivamente la eficiencia del procesador al resolver la ecuación matemática de la magnitud.

\subsection{Plataforma experimental}
El desarrollo y las pruebas se llevaron a cabo utilizando el compilador \texttt{icpx} de la suite Intel oneAPI. Se configuró un espacio de datos masivo utilizando una imagen PGM de $256 \times 256$ píxeles, replicada en memoria 800 veces para generar una carga de trabajo robusta y permitir que \textit{Intel Advisor} recolectara métricas estables.

\subsection{Procedimiento de Compilación y Configuración de Perfilado}

Para garantizar la validez matemática y la precisión de las métricas recolectadas por \textit{Intel Advisor}, el proceso de compilación requirió una configuración minuciosa de banderas (\textit{flags}) a través del compilador \texttt{icpx} de Intel oneAPI. El objetivo principal fue habilitar la recolección de métricas de hardware sin que las optimizaciones agresivas de la herramienta ofuscaran el análisis del código.

A continuación, se detallan los comandos de compilación ejecutados mediante la terminal en sistemas basados en UNIX para generar los binarios de cada variante del kernel:
...
\begin{itemize}
    \item \texttt{-g}: Genera los símbolos de depuración. Este parámetro es imperativo para que Intel Advisor logre mapear los cuellos de botella detectados en el ensamblador directamente hacia las líneas de código fuente en C++.
    \item \texttt{-march=native}: Instruye al compilador a exprimir el conjunto de instrucciones máximo soportado por la arquitectura del procesador anfitrión (host), habilitando la emisión de instrucciones AVX2.
    \item \texttt{-fno-inline}: Evita que el compilador expanda las funciones directamente en la rutina principal (\textit{main}). Esto aísla el kernel del operador Sobel, permitiendo medir exclusivamente su tiempo de ejecución sin ruido de E/S.
    \item \texttt{-qopt-report}: Genera registros textuales de máxima verbosidad (\textit{max}) sobre la fase de vectorización (\textit{vec}), crucial para comprender por qué el compilador aprueba o rechaza paralelizar un ciclo.
\end{itemize}

\subsubsection{Flujo de Trabajo en Intel Advisor}
Una vez generados los ejecutables, el análisis de rendimiento no consistió en una ejecución aislada, sino en una recolección bifásica utilizando la interfaz gráfica de \textit{Intel Advisor}:
\begin{enumerate}
    \item \textbf{Survey Analysis (Análisis de Inspección):} Se ejecutó inicialmente para medir los tiempos de ejecución base, detectar los bucles más pesados (hotspots) y generar el \textit{Instruction Mix} (mezcla dinámica de operaciones vectoriales y escalares).
    \item \textbf{Trip Counts and FLOP Analysis (Análisis de Iteraciones y Operaciones):} Posteriormente, se instrumentó el binario para contar las operaciones matemáticas exactas de punto flotante y medir el tráfico del bus de datos. Esta fase es la responsable de inyectar los datos en el modelo \textbf{Roofline}, correlacionando finalmente el rendimiento alcanzado frente a las capacidades físicas del procesador y la memoria RAM DDR4.
\end{enumerate}
\FloatBarrier
...
Esta versión representa el algoritmo tradicional secuencial. Para garantizar que el compilador no aplicara ninguna optimización por debajo de la mesa, se compiló explícitamente con la bandera \texttt{-fno-vectorize}. Esta versión sirve como el punto de referencia (speedup de 1.0x) para las demás implementaciones.
...
A continuación, se presenta la estructura del kernel utilizado:
...
En esta variante, se delegó la responsabilidad de la vectorización completamente al compilador \texttt{icpx}. A nivel de código, la única intervención fue el uso del restrictor de memoria \texttt{\_\_restrict} en los punteros de los arreglos ($G_x$, $G_y$ y $Mag$). Esto le asegura al compilador que no existe solapamiento de memoria (aliasing), dándole luz verde para generar instrucciones SIMD al compilar con el nivel de optimización \texttt{-O3}.
...
Al igual que en la versión anterior, la representación lógica del algoritmo se mantiene intacta:
...
Para forzar el paralelismo de manera estructurada sin escribir código dependiente de la arquitectura, se inyectaron directivas de OpenMP. Se colocó el pragma \texttt{\#pragma omp simd} justo antes del ciclo \texttt{for} principal. Esta técnica obliga al compilador a ignorar posibles dependencias de datos que él crea detectar y vectorizar el bucle obligatoriamente.
...
La representación lógica de esta implementación destaca la presencia de la directiva de paralelización:
...
Esta versión representa el nivel de optimización más bajo y manual. Se importó la librería \texttt{<immintrin.h>} para utilizar directamente las funciones intrínsecas del procesador. Se procesaron bloques de 8 píxeles simultáneamente utilizando registros de 256 bits, mediante operaciones de carga (\texttt{\_mm256\_loadu\_ps}), multiplicación (\texttt{\_mm256\_mul\_ps}), suma (\texttt{\_mm256\_add\_ps}), raíz cuadrada (\texttt{\_mm256\_sqrt\_ps}) y almacenamiento (\texttt{\_mm256\_storeu\_ps}).

\subsubsection{Versión Explícita (AVX2)}

Esta última iteración representa el nivel más profundo de optimización manual. Para evadir por completo las decisiones conservadoras del compilador, se reprogramó el kernel utilizando funciones intrínsecas de Intel (\texttt{<immintrin.h>}). El ciclo for principal fue rediseñado con un incremento de 8 en 8, permitiendo cargar, multiplicar, sumar y extraer la raíz cuadrada de 8 valores de tipo \textit{float} (256 bits) de manera estrictamente simultánea utilizando los registros AVX2.
...
Para contextualizar correctamente los resultados arrojados por el perfilador y entender los topes del modelo Roofline, es imperativo definir la arquitectura física anfitriona (\textit{host}). Las pruebas de estrés y validación se ejecutaron bajo el siguiente entorno de hardware y software:

\begin{itemize}
    \item \textbf{Procesador (CPU):} Arquitectura de 64 bits de alto rendimiento, dotada de múltiples núcleos físicos y soporte nativo para el conjunto de instrucciones de extensiones vectoriales avanzadas (AVX y AVX2). AMD Ryzen 9 5900X (24) @ 4.954G.
    \item \textbf{Memoria Principal (RAM):} 128 GB de capacidad DDR4 3200Mhz total de almacenamiento volátil. Como se evidenció en los perfiles, el sistema opera con un ancho de banda pico real medido de entre $29.06$ y $31.48$ GB/s.
    ...
    \item \textbf{Sistema Operativo:} Entorno basado en núcleo UNIX/Linux Ubuntu 24.04.4 LTS x86 64, garantizando una mínima sobrecarga de procesos en segundo plano durante el perfilado.
    \item \textbf{Suite de Desarrollo:} \textit{Intel oneAPI Base Toolkit} versión 2023, utilizando el compilador C++ de próxima generación (\texttt{icpx}) y el analizador de vectorización y techos de rendimiento (\textit{Intel Advisor}).
\end{itemize}
